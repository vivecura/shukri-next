...
function HeroBanner({ image, badge, title, subtitle, children, trustItems }) {
...
function RecognizeTable({ dict }) {
  ...
  const noTherapyYet = dict?.psychotherapie?.recognize?.noTherapyYet || [];
  const hadTherapy = dict?.psychotherapie?.recognize?.hadTherapy || [];
...
function MissingWaves({ dict }) {
  const t = useT();
  const waveLabelsT = dict?.psychotherapie?.missing?.waveLabels || [];
...
function MethodenKreise({ dict }) {
  const t = useT();
  const [activeNum, setActiveNum] = useState(1);
  const methodenT = dict?.psychotherapie?.methods?.items || [];
      ...
      <div className="methoden-stage">
        ...
        <div className="methoden-circles" role="tablist">
          <div className="methoden-row-top">
            {methodenData[0] && renderCircle(methodenData[0])}
          </div>
...
function Psychotherapie() {
  ...
  const faqData = dict?.psychotherapie?.faq?.items || [];

  const soArbeiteIchT = dict?.psychotherapie?.approach?.cards || [];
  ...
  const startStepsT = dict?.psychotherapie?.start?.steps || [];
  ...
  const fitItems = dict?.psychotherapie?.fit?.items || [];
  const heroMethods = dict?.psychotherapie?.hero?.methods || [];
