...
export default function LegalNotice() {
  ...
  // Fetch arrays via direct locale lookup since useT only returns string keys.
  const professionalRules = lookupRaw(lang, "legal.impressum.professionalRules.list");
  const dataTypes = lookupRaw(lang, "legal.privacy.dataTypes.list");
  const socialNetworks = lookupRaw(lang, "legal.privacy.socialNetworks.list");
  const recipients = lookupRaw(lang, "legal.privacy.recipients.list");
  const transferMechanisms = lookupRaw(lang, "legal.privacy.dataTransfer.list");
  const retentionList = lookupRaw(lang, "legal.privacy.retention.list");
  const dataSubjectRights = lookupRaw(lang, "legal.privacy.rights.list");
